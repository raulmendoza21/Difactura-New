...
async def main():
    ...
    print(f"{'ID':>3} | {'v2_num':25} | {'vis_num':25} | {'v2_base':>10} | {'vis_base':>10} | {'v2_iva%':>7} | {'vis_iva%':>7} | {'v2_total':>10} | {'vis_total':>10} | {'v2_conf':>7}")
    print("-" * 160)
    ...
    for fid, path in INVOICES:
        try:
            ...
            v2 = V2_DATA[fid]
            v2n = (v2["num"] or "")[:25]
            visn = (d["numero_factura"] or "")[:25]
            print(f"{fid:3} | {v2n:25} | {visn:25} | {v2['base']:10.2f} | {(d['base_imponible'] or 0):10.2f} | {v2['iva']:7.1f} | {(d['iva_porcentaje'] or 0):7.1f} | {v2['total']:10.2f} | {(d['total_factura'] or 0):10.2f} | {v2['conf']:7.2f}")
        except Exception as e:
            print(f"{fid:3} | ERROR: {e}")
