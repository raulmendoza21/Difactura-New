...
export default function InvoiceLineItems({ lines = [] }) {
  if (lines.length === 0) {
    return (
      <div className="card p-6">
        <h3 className="text-base font-semibold text-slate-800 mb-2">Líneas de factura</h3>
        <p className="text-sm text-slate-400">No se detectaron líneas de producto</p>
      </div>
    );
  }
  ...
  return (
    <div className="card overflow-hidden">
      <div className="px-5 py-3 border-b border-slate-100">
        <h3 className="text-base font-semibold text-slate-800">Líneas de factura</h3>
      </div>
      {/* Desktop */}
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-slate-50/80 border-b border-slate-100">
              <th className="text-left px-5 py-2.5 text-xs font-semibold text-slate-500 uppercase tracking-wider">Descripción</th>
